            ...
            {experiences.map((exp, index) => (
              ...
              >
                ...
                <div className={`ml-12 md:ml-0 md:w-5/12 ${index % 2 === 0 ? 'md:mr-auto md:pr-8' : 'md:ml-auto md:pl-8'}`}>
                  <div className={`bg-gradient-steel rounded-lg p-6 shadow-steel border-l-4 ${
                    exp.current ? 'border-industrial-accent' : 'border-industrial-blue'
                  }`}>
                    <div className="flex items-center mb-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center mr-3 ${
                        exp.current ? 'bg-industrial-accent' : 'bg-industrial-blue'
                      }`}>
                        <exp.icon className={`w-5 h-5 ${exp.current ? 'text-industrial-dark' : 'text-white'}`} />
                      </div>
                      {exp.current && (
                        <span className="bg-industrial-accent text-industrial-dark text-xs font-semibold px-2 py-1 rounded-full">
                          ATUAL
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3">
                      <h4 className="text-lg font-semibold text-industrial-blue">
                        {exp.company}
                      </h4>
                      <span className="text-sm text-industrial-steel font-medium">
                        {exp.period}
                      </span>
                    </div>
